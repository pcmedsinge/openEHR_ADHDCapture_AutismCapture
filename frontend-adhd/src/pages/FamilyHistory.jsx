          ...
          {fd.famProblems.map((p, i) => (
            <div key={i} className="repeater-row">
              <input type="text" value={p.name} onChange={e => setProblem(i, e.target.value)}
                placeholder="Condition / diagnosis (e.g. ADHD)" />
          ...
          {fd.famMembers.map((m, i) => (
            <div key={i} className="repeater-row">
              <input type="text" value={m.name} onChange={e => setMember(i,'name',e.target.value)}
                placeholder="Name or identifier (e.g. Father)" style={{ flex:2 }} />
      ...
      <div className="card">
        ...
        <div className="card-body">
          ...
          <div className="summary-grid">
            ...
            <div className="summary-chip">
              <div className="label">ASRS Part A</div>
              <div className="value">{fd.asrs.slice(0,6).reduce((s,v)=>s+v,0)} / 24</div>
            </div>
            <div className="summary-chip">
              <div className="label">ASRS Total</div>
              <div className="value">{fd.asrs.reduce((s,v)=>s+v,0)} / 72</div>
            </div>
            <div className="summary-chip">
              <div className="label">Childhood Inattention</div>
              <div className="value">{fd.diva_ci.filter(Boolean).length} / 9</div>
            </div>
            <div className="summary-chip">
              <div className="label">Adulthood Inattention</div>
              <div className="value">{fd.diva_ai.filter(Boolean).length} / 8</div>
            </div>
            <div className="summary-chip">
